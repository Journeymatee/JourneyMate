...
function BookingSheet({ open, onClose, origin, destination, type }) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className="fixed inset-0 bg-slate-950/55 dark:bg-black/70 backdrop-blur-sm z-[60]"
        onClick={onClose}
      />

      {/* Slide-up sheet */}
      <div className="fixed bottom-0 left-0 right-0 z-[70] flex flex-col bg-white dark:bg-slate-950 border-t border-slate-900/10 dark:border-white/10 rounded-t-3xl max-h-[88vh] animate-slide-up shadow-2xl shadow-slate-900/15 dark:shadow-black/60">

        {/* Drag handle */}
        <div className="flex justify-center pt-3 pb-1 shrink-0">
          <div className="w-10 h-1 bg-slate-900/15 dark:bg-white/20 rounded-full" />
        </div>

        {/* Header */}
        <div className="flex items-center justify-between gap-3 px-4 sm:px-5 py-3 border-b border-slate-900/8 dark:border-white/8 shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 shrink-0 rounded-full bg-slate-900/[0.06] hover:bg-slate-900/[0.12] dark:bg-white/8 dark:hover:bg-white/15 flex items-center justify-center text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-all active:scale-95 touch-manipulation"
          >
            <X size={18} />
          </button>
        </div>
        ...
        {/* Scrollable content */}
        <div className="overflow-y-auto overscroll-contain flex-1 px-4 sm:px-5 py-5 space-y-7 pb-[max(1.25rem,env(safe-area-inset-bottom))]">
          {sections.map((sec) => (
            <div key={sec.title}>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-lg">{sec.emoji}</span>
                <h3 className={`font-bold text-sm uppercase tracking-wider ${sec.accent === 'amber' ? 'text-amber-700 dark:text-amber-400' : 'text-green-700 dark:text-green-400'}`}>
                  {sec.title}
                </h3>
                <div className="flex-1 h-px bg-slate-900/10 dark:bg-white/6 ml-2" />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {sec.platforms.map((p) => (
                  <a
                    ...
                    className={`flex items-center justify-between gap-3 p-3.5 rounded-2xl border transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md dark:hover:shadow-lg group active:scale-[0.99] touch-manipulation ${
                      sec.accent === 'amber'
                        ? 'bg-amber-50/80 dark:bg-amber-500/5 border-amber-300/60 dark:border-amber-500/15 hover:border-amber-400/80 dark:hover:border-amber-500/35 hover:bg-amber-100/80 dark:hover:bg-amber-500/8'
                        : 'bg-green-50/80 dark:bg-green-500/5 border-green-300/60 dark:border-green-500/15 hover:border-green-400/80 dark:hover:border-green-500/35 hover:bg-green-100/80 dark:hover:bg-green-500/8'
                    }`}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                          ...
                          {p.badge && (
                            <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase whitespace-nowrap border ${
                              p.badge === 'Official'
                                ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400 border-blue-400/60 dark:border-blue-500/30'
                                : 'bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400 border-green-400/60 dark:border-green-500/30'
                            }`}>{p.badge}</span>
                          )}
                          {p.tag && !p.badge && (
                        ...
                        <p className="text-[11px] text-slate-600 dark:text-slate-500 leading-tight truncate max-w-[160px] sm:max-w-none">{p.note}</p>
                      </div>
                    </div>

                    <div className={`shrink-0 flex items-center gap-1 text-xs font-bold px-3 py-1.5 rounded-xl transition-colors ${
                      sec.accent === 'amber'
                        ? 'bg-amber-200/80 dark:bg-amber-500/15 text-amber-800 dark:text-amber-400 group-hover:bg-amber-300/90 dark:group-hover:bg-amber-500/30'
                        : 'bg-green-200/80 dark:bg-green-500/15 text-green-800 dark:text-green-400 group-hover:bg-green-300/90 dark:group-hover:bg-green-500/30'
                    }`}>
                      <span>Book</span>
          ...
          <p className="text-[10px] text-slate-400 dark:text-slate-700 text-center pb-2">
...
function PlaceHistoryModal({ open, onClose, searchQuery, preload }) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-slate-950/55 dark:bg-black/75 backdrop-blur-sm z-[80]" onClick={onClose} role="presentation" />
      <div
        className="fixed z-[90] left-1/2 top-1/2 w-[min(100vw-1rem,28rem)] sm:w-[min(100vw-2rem,32rem)] max-h-[85vh] -translate-x-1/2 -translate-y-1/2 glass border border-slate-900/10 dark:border-white/12 rounded-2xl shadow-2xl shadow-slate-900/15 dark:shadow-black/60 flex flex-col"
        role="dialog"
        aria-modal="true"
        aria-labelledby="ph-title"
      >
        <div className="flex items-start justify-between gap-3 p-4 border-b border-slate-900/8 dark:border-white/10 shrink-0">
          <div className="flex items-start gap-2 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-amber-100/80 dark:bg-amber-500/15 border border-amber-400/60 dark:border-amber-500/25 flex items-center justify-center shrink-0">
              <History size={18} className="text-amber-700 dark:text-amber-400" />
            </div>
            <h2 id="ph-title" className="text-base font-bold text-slate-900 dark:text-white leading-tight pr-1">
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-slate-900/[0.06] hover:bg-slate-900/[0.12] dark:bg-white/8 dark:hover:bg-white/15 flex items-center justify-center text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white shrink-0 active:scale-95 touch-manipulation"
            aria-label="Close"
          >
            <X size={18} />
          </button>
        ...
        <div className="overflow-y-auto overscroll-contain flex-1 p-4 min-h-0">
          {loading && (
          ...
          {err && !loading && (
            <div className="flex flex-col gap-2 text-rose-700 dark:text-rose-200/95 text-sm p-3 rounded-xl bg-rose-100/80 dark:bg-rose-500/10 border border-rose-400/50 dark:border-rose-500/20">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 pl-0.5 text-amber-700 dark:text-amber-400 text-xs font-semibold hover:text-amber-800 dark:hover:text-amber-300"
              >
                Search on Wikipedia
            ...
            <div className="space-y-3">
              {article.thumbnail && (
                <img
                  ...
                  alt=""
                  className="w-full max-h-40 object-cover rounded-xl border border-slate-900/10 dark:border-white/10"
                />
              )}
              ...
              {article.url && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-sm font-semibold text-amber-700 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300"
                >
        ...
        <p className="text-[10px] text-slate-500 dark:text-slate-600 border-t border-slate-900/8 dark:border-white/8 p-3 shrink-0">
...
function PlaceIntelSection({ placeIntel, destination, onOpenHistory, onOpenWithPreload }) {
  ...
  return (
    <div className="mb-6 sm:mb-8 rounded-2xl border border-slate-900/10 dark:border-white/10 glass p-4 sm:p-5 animate-slide-up w-full min-w-0" style={{ animationDelay: '0.12s' }}>
      <div className="flex items-center gap-2 mb-4">
        <Landmark size={16} className="text-emerald-600 dark:text-emerald-400" />
        <h3 className="text-sm font-bold text-slate-900 dark:text-white tracking-wide">Compare — live place data</h3>
      </div>

      {osrm && (
        <div className="flex flex-wrap gap-2 mb-4">
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/80 dark:bg-slate-800/80 border border-slate-900/10 dark:border-white/10 text-xs text-slate-700 dark:text-slate-200">
            <Ruler size={12} className="text-cyan-600 dark:text-cyan-400" />
            <span className="text-slate-500 dark:text-slate-500">By road (approx.):</span>
            <span className="font-semibold text-slate-900 dark:text-white tabular-nums">{osrm.distanceKm} km</span>
          </div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/80 dark:bg-slate-800/80 border border-slate-900/10 dark:border-white/10 text-xs text-slate-700 dark:text-slate-200">
            <Clock size={12} className="text-cyan-600 dark:text-cyan-400" />
            <span className="text-slate-500 dark:text-slate-500">Drive time (approx.):</span>
            <span className="font-semibold text-slate-900 dark:text-white tabular-nums">{osrm.durationMin} min</span>
          </div>
        </div>
      )}

      {wikipedia && (
        <div className="p-3 rounded-xl bg-white/70 dark:bg-white/[0.04] border border-slate-900/8 dark:border-white/8 mb-4">
          <p className="text-xs text-slate-500 dark:text-slate-500 uppercase font-semibold mb-1">About the destination</p>
          ...
          <button
            ...
            className="mt-2 text-xs font-semibold text-amber-700 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 inline-flex items-center gap-1 active:scale-[0.97] touch-manipulation"
          >
            <History size={12} />
            Read place history
          </button>
      ...
      {Array.isArray(topSights) && topSights.length > 0 && (
        <div>
          <p className="text-xs text-slate-500 dark:text-slate-500 font-semibold mb-2">Places to visit nearby (from OpenStreetMap)</p>
          <ul className="space-y-1.5">
            {topSights.map((s) => (
              <li
                key={s.name}
                className="flex items-center justify-between gap-2 text-xs sm:text-sm text-slate-700 dark:text-slate-300 pl-0"
              >
                <span className="min-w-0 truncate">{s.name}</span>
                <button
                  type="button"
                  onClick={() => onOpenHistory(`${s.name} ${destination} India`, null)}
                  className="shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-amber-100/80 dark:bg-amber-500/15 border border-amber-400/60 dark:border-amber-500/30 text-amber-700 dark:text-amber-400 text-[10px] sm:text-xs font-semibold hover:bg-amber-200/80 dark:hover:bg-amber-500/20 active:scale-[0.97] touch-manipulation"
                >
                  <History size={12} />
                  History
                </button>
...
/* ------------------------------------------------------------------ */
/*  Famous food now lives in TripInsightsBar (Weather / Music /        */
/*  Shopping / Routes / Food). The panel itself sits in                */
/*  ./StreetFoodPanel.jsx and is rendered inside an InsightModal.      */
/* ------------------------------------------------------------------ */
...
function ItineraryDay({
  ...
  return (
    <div className={`rounded-xl border transition-all duration-300 overflow-hidden ${
      isGold
        ? 'bg-amber-50/80 dark:bg-amber-500/5 border-amber-300/60 dark:border-amber-500/15 hover:border-amber-400/80 dark:hover:border-amber-500/30'
        : 'bg-green-50/80 dark:bg-green-500/5 border-green-300/60 dark:border-green-500/15 hover:border-green-400/80 dark:hover:border-green-500/30'
    } ${expanded ? 'ring-1 ring-slate-900/10 dark:ring-white/15 shadow-md dark:shadow-xl' : 'hover:scale-[1.005]'}`}>

      <button
        type="button"
        onClick={onToggle}
        className="w-full flex items-center gap-3 p-3 sm:p-4 text-left group active:bg-slate-900/[0.03] dark:active:bg-white/[0.02] touch-manipulation"
        aria-expanded={expanded}
      >
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 tabular-nums ${
          isGold ? 'bg-amber-200/80 dark:bg-amber-500/20 text-amber-800 dark:text-amber-400' : 'bg-green-200/80 dark:bg-green-500/20 text-green-800 dark:text-green-400'
        }`}>D{day.day}</div>
        <span className="font-semibold text-sm text-slate-900 dark:text-white leading-snug flex-1 min-w-0">{day.title}</span>
        {hasNote && (
          <span
            className={`hidden xs:inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-full border ${
              isGold ? 'bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-300 border-amber-300/70 dark:border-amber-500/30' : 'bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-300/70 dark:border-emerald-500/30'
            }`}
            title={note}
          >
            Note
        ...
        <ChevronDown size={16} className={`shrink-0 text-slate-500 dark:text-slate-500 transition-transform duration-300 ${expanded ? 'rotate-180' : ''} group-hover:text-slate-700 dark:group-hover:text-slate-300`} />
      </button>
          ...
          <div className="px-3 sm:px-4 pb-3 sm:pb-4">
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
              <ul className="space-y-1.5 lg:col-span-2">
                {day.activities.map((act, i) => (
                  <li key={i} className="flex items-start gap-2 text-xs text-slate-700 dark:text-slate-400">
                    <ChevronRight size={12} className={`mt-0.5 shrink-0 ${isGold ? 'text-amber-600 dark:text-amber-500' : 'text-green-600 dark:text-green-500'}`} />
                    <span className="min-w-0 flex-1 leading-snug">{act}</span>
                    {onHistoryForActivity && destinationName && (
                      <button
                        type="button"
                        onClick={() => onHistoryForActivity(act)}
                        className={`shrink-0 p-0.5 rounded border border-dashed active:scale-90 touch-manipulation ${
                          isGold ? 'border-amber-500/40 dark:border-amber-500/30 text-amber-700 dark:text-amber-400 hover:bg-amber-100/80 dark:hover:bg-amber-500/10' : 'border-green-500/40 dark:border-green-500/30 text-green-700 dark:text-green-400 hover:bg-green-100/80 dark:hover:bg-green-500/10'
                        }`}
              ...
              {day.map && (
                <div className="lg:col-span-3">
                  <div className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-500 font-semibold mb-1.5 flex items-center gap-1.5">
                    <MapPin size={10} className={isGold ? 'text-amber-700 dark:text-amber-400' : 'text-green-700 dark:text-green-400'} />
                    {day.map.label}
                  </div>
                  <div className={expanded ? 'h-[260px] sm:h-[300px]' : 'h-[200px]'}>
            ...
            {(canEditNote || hasNote) && (
              <div className={`mt-3 rounded-lg border p-3 ${
                isGold ? 'bg-amber-100/60 dark:bg-amber-500/5 border-amber-300/60 dark:border-amber-500/20' : 'bg-emerald-100/60 dark:bg-emerald-500/5 border-emerald-300/60 dark:border-emerald-500/20'
              }`}>
                <div className={`text-[10px] uppercase tracking-wider font-semibold mb-1.5 flex items-center gap-1.5 ${
                  isGold ? 'text-amber-700 dark:text-amber-300' : 'text-emerald-700 dark:text-emerald-300'
                }`}>
                  <ChefHat size={10} />
                  Personal note
                </div>
                {canEditNote ? (
                  <textarea
                    ...
                    className="w-full bg-white/90 dark:bg-white/5 border border-slate-900/10 dark:border-white/10 rounded-lg px-3 py-2 text-xs sm:text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 outline-none focus:border-slate-900/25 dark:focus:border-white/25 resize-y leading-snug"
                  />
                ) : (
                  <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-200 whitespace-pre-line leading-snug">{note}</p>
                )}
                {canEditNote && (
      ...
      {!expanded && day.map && (
        <div className="px-3 sm:px-4 pb-3 text-[11px] text-slate-500 dark:text-slate-500 flex items-center gap-1.5">
          <MapPin size={10} className={isGold ? 'text-amber-700/80 dark:text-amber-400/70' : 'text-green-700/80 dark:text-green-400/70'} />
  ...
  return (
    ...
    >
      <button
        ...
        onClick={onClose}
        tabIndex={-1}
        className="absolute inset-0 bg-slate-950/55 dark:bg-black/80 backdrop-blur-md cursor-default"
      />

      <div
        className="relative glass border border-slate-900/10 dark:border-white/15 shadow-2xl shadow-slate-900/15 dark:shadow-black/60
                   w-full max-w-2xl
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-900/8 dark:border-white/10 bg-white/85 dark:bg-slate-950/50 backdrop-blur-sm shrink-0">
          <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-br from-fuchsia-100 to-pink-100 dark:from-fuchsia-500/30 dark:to-pink-500/20 border border-fuchsia-300/70 dark:border-fuchsia-400/30 flex items-center justify-center shrink-0 text-base">
            <span aria-hidden>{tripMeta?.icon || '✨'}</span>
          </div>
          <div className="min-w-0 flex-1">
            <h3 id="travellers-modal-title" className="text-base sm:text-lg font-bold text-slate-900 dark:text-white leading-tight truncate font-display">
              ...
              {[0, 1].map((i) => (
                <span
                  ...
                  className={`h-1 rounded-full transition-all duration-300 ${
                    step === i
                      ? 'w-5 bg-fuchsia-500 dark:bg-fuchsia-300'
                      : i < step
                      ? 'w-3 bg-slate-900/45 dark:bg-white/55'
                      : 'w-3 bg-slate-900/15 dark:bg-white/15'
                  }`}
                />
              ))}
              <span className="ml-1 text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-500 font-semibold">
                Step {step + 1} / 2
              </span>
            </div>
          </div>
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="w-10 h-10 rounded-xl bg-slate-900/[0.04] hover:bg-slate-900/[0.08] dark:bg-white/5 dark:hover:bg-white/10 border border-slate-900/10 dark:border-white/10 hover:border-slate-900/20 dark:hover:border-white/20 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-all active:scale-95 touch-manipulation shrink-0"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain px-4 sm:px-6 py-5 min-h-0">
          {step === 0 ? (
            ...
            <>
              {tripMeta && (
                <div className="inline-flex items-center gap-1.5 mb-4 text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-fuchsia-100/80 dark:bg-fuchsia-500/15 text-fuchsia-700 dark:text-fuchsia-200 border border-fuchsia-300/70 dark:border-fuchsia-400/30">
                  <span aria-hidden>{tripMeta.icon}</span>
                  <span>{tripMeta.short} trip</span>
                </div>
              )}
              {!tripType ? (
                <div className="rounded-2xl border border-amber-300/70 dark:border-amber-500/20 bg-amber-50/80 dark:bg-amber-500/5 p-4 text-xs text-amber-800 dark:text-amber-200/85 leading-relaxed">
        ...
        {/* Sticky footer */}
        <div className="flex items-center justify-between gap-3 px-4 sm:px-6 py-3 sm:py-4 border-t border-slate-900/8 dark:border-white/10 bg-white/85 dark:bg-slate-950/50 backdrop-blur-sm shrink-0">
          {step === 1 ? (
            <button
              type="button"
              onClick={() => setStep(0)}
              className="inline-flex items-center gap-1.5 text-xs sm:text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-semibold transition-colors active:scale-95 touch-manipulation"
            >
              <ArrowLeft size={14} /> Back
            </button>
          ) : (
            <span className="text-[11px] text-slate-500 dark:text-slate-500 font-semibold">
          ...
          {step === 0 ? (
            <button
              type="button"
              onClick={() => setStep(1)}
              className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-2xl text-sm font-bold text-white bg-gradient-to-r from-fuchsia-500 to-pink-500 hover:from-fuchsia-400 hover:to-pink-400 shadow-lg shadow-fuchsia-500/20 transition-all hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] touch-manipulation"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-2xl text-sm font-bold text-white bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-400 hover:to-green-500 shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] touch-manipulation"
            >
              <Check size={16} /> Done
            </button>
          )}
...
/*  Plan card                                                          */
/* ------------------------------------------------------------------ */
function PlanCard({ plan, type, tripData, onBook, onHistoryForActivity, tripType = null, vibes = [], notes = {}, onNoteChange, readOnly = false }) {
  const isGold = type === 'gold'
  ...
  // Theme-aware accents: light tones for bright mode, vibrant tones for dark.
  const color = isGold ? {
    ...
    gradient: 'from-amber-100/90 to-orange-100/80 dark:from-amber-500/20 dark:to-orange-500/10',
    ...
    badge: 'bg-amber-100 dark:bg-amber-500/15 text-amber-800 dark:text-amber-400 border-amber-300/70 dark:border-amber-500/30',
    tab: 'text-amber-700 dark:text-amber-400 border-amber-500 dark:border-amber-400',
    icon: 'text-amber-700 dark:text-amber-400',
    check: 'text-amber-600 dark:text-amber-400',
    ring: 'ring-amber-300/40 dark:ring-amber-500/20',
    glow: 'shadow-amber-500/10',
    cardGlass: 'glass-gold',
  } : {
    primary: 'text-green-700 dark:text-green-400',
    bg: 'bg-green-100/60 dark:bg-green-500/10',
    border: 'border-green-300/70 dark:border-green-500/25',
    gradient: 'from-green-100/90 to-emerald-100/80 dark:from-green-500/20 dark:to-emerald-500/10',
    ...
    badge: 'bg-green-100 dark:bg-green-500/15 text-green-800 dark:text-green-400 border-green-300/70 dark:border-green-500/30',
    tab: 'text-green-700 dark:text-green-400 border-green-500 dark:border-green-400',
    icon: 'text-green-700 dark:text-green-400',
    check: 'text-green-600 dark:text-green-400',
    ring: 'ring-green-300/40 dark:ring-green-500/20',
  ...
  return (
    <div className={`plan-card ${isGold ? 'plan-card-gold' : 'plan-card-silver'} ${color.cardGlass} rounded-2xl sm:rounded-3xl overflow-hidden ring-1 ${color.ring} shadow-2xl ${color.glow} flex flex-col`}>
      ...
      <div className={`bg-gradient-to-br ${color.gradient} p-4 sm:p-6 border-b ${color.border}`}>
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
          <div className="min-w-0">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-sm mt-1 max-w-[20rem]">
              {isGold ? 'Premium comfort, curated experiences' : 'Smart savings, authentic travel'}
            </p>
          </div>
          <div className="text-left sm:text-right shrink-0">
            <div className="text-xs text-slate-500 dark:text-slate-500 mb-1">Total Cost</div>
            <div className={`font-display font-bold text-2xl sm:text-3xl ${color.primary} price-animate tabular-nums`}>
              ₹{plan.price.toLocaleString('en-IN')}
            </div>
            <div className="text-xs text-slate-500 dark:text-slate-500">per person</div>
            {tripData?.requestedDays != null && (
              <p className="text-[10px] text-slate-500 dark:text-slate-500 mt-1 max-w-[12rem] sm:max-w-none sm:text-right leading-snug">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl ${color.bg} border ${color.border}`}>
            ...
            <div className="min-w-0">
              <div className="text-xs text-slate-500 dark:text-slate-500">Transport</div>
              <div className="text-xs font-semibold text-slate-900 dark:text-white leading-tight truncate">{plan.transport}</div>
            </div>
          </div>
          <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl ${color.bg} border ${color.border}`}>
            {isGold ? <Hotel size={16} className={color.icon} /> : <Building size={16} className={color.icon} />}
            <div className="min-w-0">
              <div className="text-xs text-slate-500 dark:text-slate-500">Stay</div>
              <div className="text-xs font-semibold text-slate-900 dark:text-white leading-tight truncate">{plan.accommodation}</div>
            </div>
          </div>
        </div>

        {tunedBadge && (
          <div className="mt-2.5 flex items-start gap-2 text-[11px] text-fuchsia-700 dark:text-fuchsia-100/85 bg-fuchsia-100/80 dark:bg-fuchsia-500/10 border border-fuchsia-300/70 dark:border-fuchsia-400/25 rounded-xl px-3 py-2">
            <span className="font-bold uppercase tracking-wider text-fuchsia-700/90 dark:text-fuchsia-200/80 text-[9px] shrink-0 mt-0.5">Tuned for</span>
      ...
      {/* Tabs */}
      <div className="flex border-b border-slate-900/8 dark:border-white/8 min-w-0">
        {tabs.map(tab => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`flex-1 min-w-0 py-2.5 sm:py-3 px-0.5 sm:px-1 text-[9px] 2xs:text-[10px] sm:text-xs font-semibold uppercase tracking-tight sm:tracking-wider transition-all duration-200 active:scale-[0.97] touch-manipulation ${
              activeTab === tab
                ? `${color.tab} border-b-2`
                : 'text-slate-500 hover:text-slate-700 dark:text-slate-500 dark:hover:text-slate-300 border-b-2 border-transparent'
            }`}
          >
      ...
      <div className="flex-1 p-4 sm:p-5 overflow-auto min-h-0">
          ...
          <div className="space-y-4 animate-fade-in">
            {[
              ...
              <div key={label} className={`flex gap-3 p-3 rounded-xl ${color.bg} border ${color.border}`}>
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-sm text-slate-900 dark:text-white leading-snug break-words">{value}</div>
                </div>
              </div>
            ))}
            <div className={`flex items-center gap-3 p-3 rounded-xl ${color.bg} border ${color.border}`}>
              ...
              <div className="min-w-0">
                ...
                <div className="text-sm text-slate-900 dark:text-white">{tripData.duration}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'itinerary' && (
          <div className="animate-fade-in">
            {/* Famous food moved to the top "Insights" bar (Weather / Music /
                Shopping / Route / Food) so the itinerary stays focused on
                day-by-day plans. */}
            <div className="space-y-3">
        ...
        {activeTab === 'book' && (
          <div className="space-y-5 animate-fade-in">
            <p className="text-xs text-slate-600 dark:text-slate-500 leading-relaxed">
              Choose a platform below to book your {isGold ? 'luxury' : 'budget'} trip from <strong className="text-slate-900 dark:text-white">{tripData.origin}</strong> to <strong className="text-slate-900 dark:text-white">{tripData.destination}</strong>. Links marked <span className={`font-semibold ${isGold ? 'text-amber-700 dark:text-amber-400' : 'text-green-700 dark:text-green-400'}`}>Pre-filled</span> open directly with your cities already selected.
            </p>
            ...
            {buildPlatforms(tripData.origin, tripData.destination, type).map((sec) => (
              <div key={sec.title}>
                <div className="flex items-center gap-2 mb-2.5">
                  <span>{sec.emoji}</span>
                  <span className={`text-xs font-bold uppercase tracking-wider ${sec.accent === 'amber' ? 'text-amber-700 dark:text-amber-400' : 'text-green-700 dark:text-green-400'}`}>{sec.title}</span>
                </div>
                <div className="space-y-2">
                  {sec.platforms.map((p) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`flex items-center justify-between gap-3 px-4 py-3 rounded-xl border transition-all hover:-translate-y-0.5 group active:scale-[0.99] touch-manipulation ${
                        sec.accent === 'amber'
                          ? 'bg-amber-50/80 dark:bg-amber-500/5 border-amber-300/60 dark:border-amber-500/15 hover:border-amber-400/80 dark:hover:border-amber-500/30'
                          : 'bg-green-50/80 dark:bg-green-500/5 border-green-300/60 dark:border-green-500/15 hover:border-green-400/80 dark:hover:border-green-500/30'
                      }`}
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {p.badge && (
                              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400 border border-blue-400/60 dark:border-blue-500/30 font-bold uppercase">{p.badge}</span>
                            )}
                            {p.tag && !p.badge && (
                              <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase ${p.tagBg}`}>{p.tag}</span>
                            )}
                          </div>
                          <p className="text-[11px] text-slate-600 dark:text-slate-500 truncate">{p.note}</p>
                        </div>
                      </div>
                      <div className={`shrink-0 flex items-center gap-1 text-xs font-bold px-2.5 py-1.5 rounded-lg ${
                        sec.accent === 'amber' ? 'bg-amber-200/80 dark:bg-amber-500/20 text-amber-800 dark:text-amber-400' : 'bg-green-200/80 dark:bg-green-500/20 text-green-800 dark:text-green-400'
        ...
        {activeTab === 'perks' && (
          <div className="space-y-2 animate-fade-in">
            <div className="text-xs text-slate-600 dark:text-slate-500 mb-4 uppercase tracking-wider font-semibold flex items-center gap-2">
              <span>What&apos;s Included</span>
              {tunedBadge && (
                <span className="ml-auto inline-flex text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-fuchsia-100/80 dark:bg-fuchsia-500/15 text-fuchsia-700 dark:text-fuchsia-200 border border-fuchsia-300/70 dark:border-fuchsia-400/30">
            ...
            {(plan.perks || []).map((perk, i) => (
              ...
              >
                ...
                <span className="text-sm text-slate-900 dark:text-white">{perk}</span>
      ...
      {/* CTA */}
      <div className="p-4 sm:p-5 border-t border-slate-900/8 dark:border-white/8">
        <button
          type="button"
          onClick={onBook}
          className={`w-full py-3.5 sm:py-4 rounded-2xl font-bold text-sm text-white transition-all duration-200 shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.99] touch-manipulation ${color.button}`}
        >
...
export default function ComparisonPage({
  ...
  // Famous food now opens via the TripInsightsBar tile (alongside Weather,
  // Music, Shopping, and Route) — no separate page-level modal state needed.
      ...
      <div className="max-w-7xl 3xl:max-w-[1680px] 4xl:max-w-[2000px] mx-auto w-full min-w-0">
        ...
        >
          <div className="mb-3.5 flex items-center gap-2.5 flex-wrap">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-violet-400 to-indigo-500 text-white shadow-md shadow-violet-500/25 dark:shadow-lg dark:shadow-violet-500/30 ring-1 ring-white/40 dark:ring-white/15">
              <SlidersHorizontal size={16} strokeWidth={2.4} />
            </div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-violet-700 dark:text-violet-300/90">
                Personalise
              </p>
              <h3
                className="text-base sm:text-lg font-semibold text-slate-900 dark:text-white tracking-tight"
            ...
            {activeTripTypeMeta && (
              <span className="ml-auto inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full bg-fuchsia-100/80 dark:bg-fuchsia-500/15 text-fuchsia-700 dark:text-fuchsia-200 border border-fuchsia-300/70 dark:border-fuchsia-400/30">
          ...
          <div className="w-full min-w-0 flex flex-col gap-4">
            ...
            <div className="w-full min-w-0 pb-1 border-b border-slate-900/8 dark:border-white/8">
              <div className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-500 font-semibold mb-2">
                Travellers &amp; vibe
              </div>
              <button
                type="button"
                onClick={() => setTravellersModalOpen(true)}
                className="w-full min-w-0 group flex items-center gap-3 px-3 sm:px-4 py-3 rounded-2xl border bg-white/70 dark:bg-white/5 hover:bg-white dark:hover:bg-white/8 border-slate-900/10 dark:border-white/10 hover:border-fuchsia-400/60 dark:hover:border-fuchsia-400/40 transition-all text-left active:scale-[0.99] touch-manipulation"
              >
                <span
                  className="shrink-0 w-9 h-9 rounded-xl bg-gradient-to-br from-fuchsia-100 to-pink-100 dark:from-fuchsia-500/30 dark:to-pink-500/20 border border-fuchsia-300/70 dark:border-fuchsia-400/30 flex items-center justify-center text-base"
                  aria-hidden
                >
                  {activeTripTypeMeta?.icon || '✨'}
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block text-xs sm:text-sm font-semibold text-slate-900 dark:text-white truncate">
                    {activeTripTypeMeta ? `${activeTripTypeMeta.label}` : 'Choose travellers & vibe'}
                  </span>
                  <span className="block text-[11px] text-slate-600 dark:text-slate-400 truncate">
                    {activeTripTypeMeta
                ...
                <span className="shrink-0 inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider px-2 py-1 rounded-full bg-fuchsia-100/80 dark:bg-fuchsia-500/15 text-fuchsia-700 dark:text-fuchsia-200 border border-fuchsia-300/70 dark:border-fuchsia-400/30 group-hover:bg-fuchsia-200/80 dark:group-hover:bg-fuchsia-500/20">
                  Edit
            ...
            <div className="w-full min-w-0">
              ...
              <div
                className="grid grid-cols-3 gap-1 w-full min-w-0 p-1 rounded-2xl bg-slate-100/80 dark:bg-slate-900/60 border border-slate-900/10 dark:border-white/10"
                style={{ maxWidth: '100%' }}
              >
                {[
                  ...
                  <button
                    ...
                    className={`min-w-0 py-2.5 sm:py-2.5 px-1 sm:px-2 rounded-xl text-xs sm:text-sm font-semibold text-center leading-snug transition-all duration-300 active:scale-[0.97] touch-manipulation ${
                      planView === opt.id
                        ? `bg-gradient-to-r ${opt.color} text-white shadow-md dark:shadow-lg`
            ...
            {onChangeDays && (
              <div className="w-full min-w-0 space-y-2 pt-1 border-t border-slate-900/8 dark:border-white/8">
                <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-1.5 gap-y-0 pt-3">
                  ...
                  {daysLoading && (
                    <span className="text-xs text-slate-500 dark:text-slate-500 flex items-center gap-1.5 sm:justify-end">
                ...
                >
                  {DAY_OPTIONS.map((d) => (
                    <button
                      ...
                      className={`min-h-11 min-w-0 flex items-center justify-center rounded-lg text-xs sm:text-sm font-bold transition-all border touch-manipulation tabular-nums ${
                        selectedDays === d
                          ? 'bg-cyan-100 dark:bg-cyan-500/25 border-cyan-500/60 dark:border-cyan-400/50 text-cyan-800 dark:text-cyan-200'
                          : 'bg-white/70 dark:bg-white/5 border-slate-900/10 dark:border-white/10 text-slate-700 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:border-slate-900/20 dark:hover:border-white/20 active:scale-95'
        ...
        >
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-400 to-sky-500 text-white shadow-md shadow-cyan-500/25 dark:shadow-lg dark:shadow-cyan-500/30 ring-1 ring-white/40 dark:ring-white/15">
            <Scale size={16} strokeWidth={2.4} />
          </div>
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-cyan-700 dark:text-cyan-300/90">
              Side by side
            </p>
            <h2
              className="text-base sm:text-lg font-semibold text-slate-900 dark:text-white tracking-tight"
          ...
          <div className="flex-1 h-px bg-gradient-to-r from-slate-900/10 via-slate-900/5 to-transparent dark:from-white/10 dark:via-white/5 dark:to-transparent ml-1" />
          {planView === 'both' && Number.isFinite(savings) && savings > 0 && (
            <span className="hidden xs:inline-flex items-center gap-1.5 rounded-full border border-emerald-400/60 dark:border-emerald-400/35 bg-emerald-100/80 dark:bg-emerald-500/15 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-emerald-800 dark:text-emerald-200 shadow-sm dark:shadow-emerald-500/20 tabular-nums">
